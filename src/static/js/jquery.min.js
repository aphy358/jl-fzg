...
// 当第四个参数传入 'noAnimation' 或第五个参数传入值时，则不显示数据加载动效
jQuery.each(["get","post"],function(i,method){jQuery[method]=function(url,data,callback,type,noAnimation){if(jQuery.isFunction(data)){type=type||callback;callback=data;data=undefined}var params={url:url,type:method,dataType:(type!=='noAnimation'?type:null),data:data,success:callback};if( !(type === 'noAnimation' || noAnimation) ){var layLoading = layer.load();params.complete = function(){layer.close(layLoading);};}return jQuery.ajax(jQuery.extend(params,jQuery.isPlainObject(url)&&url))}});


// 兼容自定义的 select 组件点击小三角无法显示下拉框的问题
$(document).delegate('i.drag-down', 'mousedown', function(){ $(this).prev('*').trigger('mousedown'); })
